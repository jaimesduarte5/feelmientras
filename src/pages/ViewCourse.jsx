...
const ViewCourse = () => {
  ...
  const { isLoading, activities, course } = useSelector(
    (state) => state.viewerCourse
  );
  //const { course } = useSelector((state) => state.contentManagePoc);
  ...
  useEffect(() => {
    ...
    const idCampaign = decryptURL(paramurl.idCampaign);
